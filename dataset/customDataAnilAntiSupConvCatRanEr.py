...
class customData(Dataset):
        ...
        with open(txt_path) as input_file:
            ...
            if phase=='meta_train':
                for line in lines[:lengthOfTrain]:
                    ...
                    ran_erase_path = path.replace("trainSquareCropped","trainSquareRanEra_mask")
                    label = int(line.strip().split(' ')[-1])
                    if os.path.isfile(ran_erase_path):
    ...
    def __getitem__(self, item):
        try:
            ...
            ran_erase_img_name = img_name.replace("trainSquareCropped","trainSquareRanEra_mask")
            img_aug = self.loader(ran_erase_img_name)

            if self.data_transforms is not None:
                try:
                    if self.phase != 'test':
                        imgs = self.data_transforms(img)
                        imgs_aug = self.data_transforms(img_aug)
                        cat_img_aug = [imgs, imgs_aug]
                    else:
                        imgs = self.data_transforms(img)
                        cat_img_aug = imgs
                    # imgs = [self.data_transforms(img),self.data_transforms(img)]
                except:
                    print("Cannot transform image: {}".format(img_name))
            return cat_img_aug, label
        except:
            print(item)
