...
class Head(nn.Module):
    ...
    def __init__(self, out_feature):
        ...
        self.head = torch.nn.Linear(out_feature, 2)
        self.projHead = torch.nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Flatten(start_dim=1), nn.Dropout(0.2))
...
def main(args):
    ...
    # head = torch.nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Flatten(start_dim=1), nn.Dropout(0.2), head)
    head = Head(out_feature=out_feature)
    ...
    for iteration in range(args.iters):#20000
        ...
        if iteration<100 or iteration%1000==0:
            TestOri(args, features, learner, test_data_loader, iteration, withNorm=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results_path', type=str, default="output/anil_anti_circle_cross_norm/", metavar='S',
                        help='output dir')
