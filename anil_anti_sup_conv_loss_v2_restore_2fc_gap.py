...
def fast_adapt_supcon(batch,
    ...
    valid_error_sup_conv = loss_supcon(evaluation_data, evaluation_labels)
...
def main(args):
    ...
    head =  torch.nn.Sequential(nn.Linear(out_feature, out_feature),
                                nn.ReLU(inplace=True),
                                nn.Linear(out_feature, args.ways))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results_path', type=str, default="output/check_for_supconv_v2_wo_linear_in_sequtial_wo_norm_2fc_gap/", metavar='S',
                        help='output dir')
