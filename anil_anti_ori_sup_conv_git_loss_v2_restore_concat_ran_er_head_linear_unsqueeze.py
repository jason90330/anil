...
from dataset.customDataAnilAntiSupConvCatRanEr import customData
from torchvision import transforms
from misc.test import TestSupConvAug
from dataset.augClass import RandomErasing
...
class FeatExtr(nn.Module):
    ...
    def __init__(self, feat_dim=128):
        ...
        self.projHead =  torch.nn.Sequential(
                                        ...
                                        nn.Dropout(0.2),
                                        nn.Linear(self.out_feature, int(self.out_feature/2)),
                                        nn.ReLU(inplace=True),
                                        nn.Linear(int(self.out_feature/2), feat_dim))
...
def fast_adapt_supcon(batch,
    ...
    imgs, imgs_aug = data[0].to(device), data[1].to(device)
    data = features(imgs)
    data_aug = features(imgs_aug)
    ...
    adaptation_data, adaptation_data_aug, adaptation_labels = data[adaptation_indices], data_aug[adaptation_indices], labels[adaptation_indices]
    evaluation_data, evaluation_data_aug, evaluation_labels = data[evaluation_indices], data_aug[evaluation_indices], labels[evaluation_indices]
    # adaptation_data_cat_sup = torch.cat([adaptation_data.unsqueeze(1), adaptation_data_aug.unsqueeze(1)], dim=1)
    evalulation_data_cat_sup = torch.cat([evaluation_data.unsqueeze(1), evaluation_data_aug.unsqueeze(1)], dim=1)
    adaptation_data_cat = torch.cat((adaptation_data, adaptation_data_aug), 1)
    evalulation_data_cat = torch.cat((evaluation_data, evaluation_data_aug), 1)

    for step in range(adaptation_steps):
        out = learner(adaptation_data_cat)
        train_error = loss_cross(out, adaptation_labels)
        learner.adapt(train_error)

    # for cross entropy input
    predictions = learner(evalulation_data_cat)
    ...
    valid_error_sup_conv = loss_supcon(evalulation_data_cat_sup, evaluation_labels)
...
def main(args):
    ...
    pre_processRGB_aug = transforms.Compose([
                                    # transforms.RandomResizedCrop(size=224, scale=(0.8, 1.)),
                                    transforms.RandomHorizontalFlip(),
                                    # transforms.RandomApply([
                                    #     transforms.ColorJitter(0.1, 0.1, 0.1, 0.1)
    ...
    head = torch.nn.Linear(256, args.ways)
    # out_feature = features.out_feature *2
    ...
    for iteration in range(args.iters):#20000
        ...
        if iteration<100 or iteration%500==0:
            TestSupConvAug(args, features, learner, test_data_loader, iteration)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results_path', type=str, default="output/anil_anti__sup_conv_loss_v2_cat_ran_er_head_linear/", metavar='S',
                        help='output dir')
