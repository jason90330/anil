...
# from loss.proxy_loss import convert_label_to_similarity, ProxyLoss
...
def log(trainErr, trainProxyErr, trainAcc, valErr, valProxyErr, valAcc, testErr, testProxyErr, testAcc):
    wandb.log({"Meta Train Error": trainErr,
               "Meta Train Proxy Error": trainProxyErr,
               "Meta Train Accuracy": trainAcc,
               "Meta Valid Error": valErr,
               "Meta Valid Proxy Error": valProxyErr,
               "Meta Valid Accuracy": valAcc,
               "Meta Test Error": testErr,
               "Meta Test Proxy Error": testProxyErr,
...
def fast_adapt(batch,
    ...
    valid_error_proxy = loss_proxy(evaluation_feats, evaluation_labels)
    valid_error_cross = loss_cross(predictions, evaluation_labels)
    valid_accuracy = accuracy(predictions, evaluation_labels)
    return valid_error_proxy, valid_error_cross, valid_accuracy
...
def main(args):
    ...
    # loss_proxy = ProxyLoss(m=0.25, gamma=80)
    # loss_proxy = losses.ProxyLoss(m=0.4, gamma=80)
    # loss_proxy = losses.ProxyLoss(m=0.25, gamma=80)
    # loss_proxy = losses.ProxyLoss(m=0.25, gamma=256)
    # loss_proxy = losses.ProxyLoss(m=0.25, gamma=60)
    loss_proxy = losses.ProxyAnchorLoss(args.ways, out_feature, margin = 0.1, alpha = 32)
    ...
    for iteration in range(args.iters):#20000
        ...
        meta_train_proxy_error = 0.0
        meta_train_accuracy = 0.0
        meta_valid_error = 0.0
        meta_valid_proxy_error = 0.0
        meta_valid_accuracy = 0.0
        meta_test_error = 0.0
        meta_test_proxy_error = 0.0
        meta_test_accuracy = 0.0
        for task in range(args.meta_bsz):#32
            # Compute meta-training loss
            learner = head.clone()
            batch = train_tasks.sample()#20
            valid_error_proxy, valid_error_cross, evaluation_accuracy = fast_adapt(batch,
                                                               learner,
                                                               features,
                                                               loss_proxy,
                                                               loss_cross,
                                                               ...
                                                               args.ways,
                                                               device)
            evaluation_error = valid_error_proxy + valid_error_cross
            evaluation_error.backward()
            meta_train_error += evaluation_error.item()
            meta_train_proxy_error += valid_error_proxy.item()
            ...
            meta_valid_proxy_error += valid_error_proxy.item()
            ...
            meta_test_proxy_error += valid_error_proxy.item()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results_path', type=str, default="output/anil_anti_proxy/", metavar='S',
                        help='output dir')
