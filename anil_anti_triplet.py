...
def log(trainErr, trainTripletErr, trainAcc, valErr, valTripletErr, valAcc, testErr, testTripletErr, testAcc):
    wandb.log({"Meta Train Error": trainErr,
               "Meta Train Triplet Error": trainTripletErr,
               "Meta Train Accuracy": trainAcc,
               "Meta Valid Error": valErr,
               "Meta Valid Triplet Error": valTripletErr,
               "Meta Valid Accuracy": valAcc,
               "Meta Test Error": testErr,
               "Meta Test Triplet Error": testTripletErr,
               "Meta Test Accuracy": testAcc})

# 從 5 round adap step 就讓 learner 依照 triplet, circle 等等 loss 改變
def fast_adapt_triplet_v2(batch, 
               learner,
               features,
               miner,
               loss_triplet,
    ...
    adaptation_embds = tmpHead(adaptation_data)
    for step in range(adaptation_steps):
        out = learner(adaptation_data)
        hard_pairs = miner(adaptation_embds, adaptation_labels)
        train_error_triplet = loss_triplet(adaptation_embds, adaptation_labels, hard_pairs)
        train_error_cross = loss_cross(out, adaptation_labels)
        train_error = train_error_triplet + train_error_cross
        learner.adapt(train_error)        

    # for cross entropy input
    predictions = learner(evaluation_data)
    # for triplet input
    evaluation_embds = tmpHead(evaluation_data)
    ...
    # Add triplet loss and cross_entropy loss
	# loss = loss_func(embeddings, labels, hard_pairs)
	# loss.backward()
	# optimizer.step()
    # Add triplet loss and cross_entropy loss
    hard_pairs = miner(evaluation_embds, evaluation_labels)
    valid_error_triplet = loss_triplet(evaluation_embds, evaluation_labels, hard_pairs)
    valid_error_cross = loss_cross(predictions, evaluation_labels)

    valid_accuracy = accuracy(predictions, evaluation_labels)
    return valid_error_triplet, valid_error_cross, valid_accuracy

def fast_adapt_triplet(batch,
               learner,
               features,
               miner,
               loss_triplet,
...
def main(args):
    ...
    loss = losses.tripletLoss(temperature=0.1, reducer=reducer)
    '''
    
    loss_cross = nn.CrossEntropyLoss(reduction='mean')
    # loss_triplet = losses.tripletLoss(temperature=0.1)
    ...
    for iteration in range(args.iters):#20000
        ...
        meta_train_triplet_error = 0.0
        meta_train_accuracy = 0.0

        meta_valid_error = 0.0
        meta_valid_triplet_error = 0.0
        meta_valid_accuracy = 0.0

        meta_test_error = 0.0
        meta_test_triplet_error = 0.0
        meta_test_accuracy = 0.0
        for task in range(args.meta_bsz):#32
            # Compute meta-training loss
            learner = head.clone()
            batch = train_tasks.sample()#20
            valid_error_triplet, valid_error_cross, evaluation_accuracy = fast_adapt_triplet_v2(batch,
                                                               learner,
                                                               features,
                                                               miner,
                                                               loss_triplet,
                                                               ...
                                                               args.ways,
                                                               device)
            evaluation_error = valid_error_triplet + valid_error_cross
            evaluation_error.backward()
            meta_train_error += evaluation_error.item()
            meta_train_triplet_error += valid_error_triplet.item()
            ...
            meta_valid_triplet_error += valid_error_triplet.item()
            ...
            meta_test_triplet_error += valid_error_triplet.item()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results_path', type=str, default="output/anil_anti_triplet_loss_inner_w_triplet/", metavar='S',
                        help='output dir')
