...
# 從 5 round adap step 就讓 learner 依照 sup_conv, circle 等等 loss 改變
def fast_adapt_sup_conv_v2(batch, 
               learner,
               features,
               miner,
               loss_sup_conv,
    ...
    for step in range(adaptation_steps):
        ...
        train_error_sup_conv = loss_sup_conv(adaptation_embds, adaptation_labels, hard_pairs)
        train_error_cross = loss_cross(out, adaptation_labels)
        train_error = train_error_sup_conv + train_error_cross
    ...
    # Add sup_conv loss and cross_entropy loss
	# loss = loss_func(embeddings, labels, hard_pairs)
	# loss.backward()
	# optimizer.step()
    # Add sup_conv loss and cross_entropy loss
    hard_pairs = miner(evaluation_embds, evaluation_labels)
    valid_error_sup_conv = loss_sup_conv(evaluation_embds, evaluation_labels, hard_pairs)
...
def main(args):
    ...
    loss = losses.sup_convLoss(temperature=0.1, reducer=reducer)
    '''
    
    loss_cross = nn.CrossEntropyLoss(reduction='mean')
    # loss_sup_conv = losses.sup_convLoss(temperature=0.1)
    miner = miners.MultiSimilarityMiner()
    loss_sup_conv = losses.SupConLoss(temperature=0.1)
    ...
    for iteration in range(args.iters):#20000
        ...
        meta_train_sup_conv_error = 0.0
        meta_train_accuracy = 0.0

        meta_valid_error = 0.0
        meta_valid_sup_conv_error = 0.0
        meta_valid_accuracy = 0.0

        meta_test_error = 0.0
        meta_test_sup_conv_error = 0.0
        meta_test_accuracy = 0.0
        for task in range(args.meta_bsz):#32
            # Compute meta-training loss
            learner = head.clone()
            batch = train_tasks.sample()#20
            valid_error_sup_conv, valid_error_cross, evaluation_accuracy = fast_adapt_sup_conv_v2(batch,
                                                               learner,
                                                               features,
                                                               miner,
                                                               loss_sup_conv,
            ...
            meta_train_sup_conv_error += valid_error_sup_conv.item()
            ...
            meta_valid_sup_conv_error += valid_error_sup_conv.item()
            ...
            meta_test_sup_conv_error += valid_error_sup_conv.item()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results_path', type=str, default="output/anil_anti_sup_conv_loss_v3_miner_hard/", metavar='S',
                        help='output dir')
